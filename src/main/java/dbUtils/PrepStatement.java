...
public class PrepStatement {
     ...
     * was also provided as input. If an exception is thrown (because the sql does
     * not compile
     * against the open database of the connection object), an error message is
     * written
    ...
    // this.errorMsg contains all the error messages that ever occured in this
    // object (all
    ...
    public String setBoolean(int position, Boolean newBoolean) {
        try {
            if (newBoolean == null) {
                ps.setNull(position, java.sql.Types.BOOLEAN);
            } else {
                this.ps.setBoolean(position, newBoolean);
            }
            return ""; // no error
        } catch (Exception e) {
            String msg = " PrepStatement: Exception in setBoolean(). Sql is " + this.sql
                    + ", position: " + position + ". Error Msg: " + e.getMessage();
            System.out.println("====> " + msg);
            this.errorMsg += msg;
            return msg;
